...
//===--------- Start of ONNXConvOpToCUDNN ----------===//

#include "mlir/Dialect/SCF/IR/SCF.h"
#include "mlir/Transforms/DialectConversion.h"

#include "src/Conversion/ONNXToCUDNN/ONNXToCUDNNCommon.hpp"
#include "src/Conversion/ONNXToCUDNN/ONNXToCUDNNCommonCUDA.cuh"
...
struct ONNXConvOpToCUDNN : public ConversionPattern {
  ONNXConvOpToCUDNN(TypeConverter &typeConverter, MLIRContext *context)
  ...
  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    auto mallocWorkspace = rewriter.create<CUDNNMallocOp>(loc, workspaceMemRefType, workspaceConst);

    auto int64Ty = rewriter.getIntegerType(64);
    auto sizeConst = emitConstantOp(rewriter, loc, int64Ty, sizeBytes);
    auto mallocConv = rewriter.create<CUDNNMallocOp>(loc, memRefType, sizeConst);

    // Convolution body
    auto convFwd = rewriter.create<CUDNNConvForwardOp>(loc, memRefType,
    ...
    if(biasMemRef) {
      ...
      addOp = rewriter.create<CUDNNAddOp>(loc, memRefType,
...
void populateLoweringONNXConvOpToCUDNNPattern(
  ...
  patterns.insert<ONNXConvOpToCUDNN>(typeConverter, context);
}
