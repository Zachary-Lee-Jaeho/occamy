...
struct ONNXReluOpToCUDNN : public ConversionPattern {
  ONNXReluOpToCUDNN(TypeConverter &typeConverter, MLIRContext *context)
    : ConversionPattern(mlir::ONNXReluOp::getOperationName(), 1, context) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    auto loc = op->getLoc();
    ONNXReluOpAdaptor operandAdaptor(operands);
    ...
    auto cudnnActivateFwd =
      rewriter.create<CUDNNActivationForwardOp>(
          ...
          /* RELU = 1 */ rewriter.getI32IntegerAttr(1));
...
void populateLoweringONNXReluOpToCUDNNPattern(
  ...
  patterns.insert<ONNXReluOpToCUDNN>(typeConverter, context);
}
//===---------- End of ONNXReluOpToCUDNN -----------===//
