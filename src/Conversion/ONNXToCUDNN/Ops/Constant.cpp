...
//===--------- Start of ONNXConstantOpToCUDNN ----------===//
...
struct ONNXConstantOpToCUDNN : public ConversionPattern {

  ONNXConstantOpToCUDNN(TypeConverter &typeConverter, MLIRContext *context)
  ...
  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    Value constantGlobal = create.krnl.constant(memRefType,
        "constant_cudnn_", constantOp.getValue().value());
    ...
    auto outMalloc = rewriter.create<CUDNNMallocOp>(loc, memRefType, sizeConst);
...
void populateLoweringONNXConstantOpToCUDNNPattern(
  ...
  patterns.insert<ONNXConstantOpToCUDNN>(typeConverter, context);
}
//===---------- End of ONNXConstantOpToCUDNN -----------===//
