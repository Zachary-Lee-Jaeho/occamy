...
//===------ CUDNNRuntimeAPI.hpp - Declaration of the Runtime API ----------===//
//
...
class CUDNNRuntimeAPIRegistry;

/// \class CUDNNRuntimeAPI
/// Represents a Runtime API callable by the compiler.
/// Instances of this class can only be created by the CUDNNRuntimeAPIRegistry
/// class.
class CUDNNRuntimeAPI final {
  friend class CUDNNRuntimeAPIRegistry;
...
public:
  ...
  static mlir::Value callApi(mlir::OpBuilder &builder, mlir::Location loc,
      const CUDNNRuntimeAPIRegistry &registry, API apiId,
      llvm::ArrayRef<mlir::Value> params);

private:
  CUDNNRuntimeAPI(API id, const std::string &name, mlir::Type outputTy,
...
class CUDNNRuntimeAPIRegistry final {
public:
  using ApiRegistry = std::map<CUDNNRuntimeAPI::API, CUDNNRuntimeAPI>;

  CUDNNRuntimeAPIRegistry(mlir::ModuleOp &module, mlir::OpBuilder &builder, mlir::Type inType);
  ~CUDNNRuntimeAPIRegistry();

  static const CUDNNRuntimeAPIRegistry build(
      mlir::ModuleOp &module, mlir::OpBuilder &builder);

  const CUDNNRuntimeAPI &getAPI(CUDNNRuntimeAPI::API apiId) const {
