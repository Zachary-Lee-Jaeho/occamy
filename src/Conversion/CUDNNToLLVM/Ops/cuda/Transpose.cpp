//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNTransposeOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNTransposeOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNTransposeOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNTransposeOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    CUDNNTransposeOp transposeOp = dyn_cast<CUDNNTransposeOp>(op);
    ...
    if (inputRank == 2) {
      if(llvmElemType.dyn_cast_or_null<IntegerType>()) {
        callTranspose = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
            apiRegistry, CUDNNRuntimeAPI::API::CUDA_TRANSPOSE_2D_I64,
            {extractInput, inputDim, extractOutput, outputDim, permArg, rankConst});
      } else if(llvmElemType.dyn_cast_or_null<FloatType>()) {
        callTranspose = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
            apiRegistry, CUDNNRuntimeAPI::API::CUDA_TRANSPOSE_2D_F32,
            {extractInput, inputDim, extractOutput, outputDim, permArg, rankConst});
      }
    } else if (inputRank == 3) {
      if(llvmElemType.dyn_cast_or_null<IntegerType>()) {
        callTranspose = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
            apiRegistry, CUDNNRuntimeAPI::API::CUDA_TRANSPOSE_3D_I64,
            {extractInput, inputDim, extractOutput, outputDim, permArg, rankConst});
      } else if(llvmElemType.dyn_cast_or_null<FloatType>()) {
        callTranspose = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
            apiRegistry, CUDNNRuntimeAPI::API::CUDA_TRANSPOSE_3D_F32,
            {extractInput, inputDim, extractOutput, outputDim, permArg, rankConst});
      }
    } else if (inputRank == 4) {
      if(llvmElemType.dyn_cast_or_null<IntegerType>()) {
        callTranspose = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
            apiRegistry, CUDNNRuntimeAPI::API::CUDA_TRANSPOSE_4D_I64,
            {extractInput, inputDim, extractOutput, outputDim, permArg, rankConst});
      } else if(llvmElemType.dyn_cast_or_null<FloatType>()) {
        callTranspose = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
            apiRegistry, CUDNNRuntimeAPI::API::CUDA_TRANSPOSE_4D_F32,
            {extractInput, inputDim, extractOutput, outputDim, permArg, rankConst});
      }
    } else if (inputRank == 6) {
      if(llvmElemType.dyn_cast_or_null<IntegerType>()) {
        callTranspose = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
            apiRegistry, CUDNNRuntimeAPI::API::CUDA_TRANSPOSE_6D_I64,
            {extractInput, inputDim, extractOutput, outputDim, permArg, rankConst});
      } else if(llvmElemType.dyn_cast_or_null<FloatType>()) {
        callTranspose = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
            apiRegistry, CUDNNRuntimeAPI::API::CUDA_TRANSPOSE_6D_F32,
            {extractInput, inputDim, extractOutput, outputDim, permArg, rankConst});
      }
    } else {
...
void mlir::populateCUDNNTransposeToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter) {
  patterns.insert<CUDNNTransposeOpLowering>(ctx, typeConverter);
}
