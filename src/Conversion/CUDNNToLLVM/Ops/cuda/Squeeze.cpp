//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNSqueezeOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNSqueezeOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNSqueezeOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNSqueezeOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    CUDNNSqueezeOp squeezeOp = dyn_cast<CUDNNSqueezeOp>(op);
    ...
    if(llvmElemType.dyn_cast_or_null<IntegerType>()) {
      callSqueeze = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
          apiRegistry, CUDNNRuntimeAPI::API::CUDA_SQUEEZE_I64,
          {extractInput, inputDim, extractOutput, outputDim, axes, rankConst});
    } else if(llvmElemType.dyn_cast_or_null<FloatType>()) {
      callSqueeze = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
          apiRegistry, CUDNNRuntimeAPI::API::CUDA_SQUEEZE_F32,
          {extractInput, inputDim, extractOutput, outputDim, axes, rankConst});
    }
...
void mlir::populateCUDNNSqueezeToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter) {
  patterns.insert<CUDNNSqueezeOpLowering>(ctx, typeConverter);
}
