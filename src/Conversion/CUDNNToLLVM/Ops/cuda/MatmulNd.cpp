//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNMatmulNdOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNMatmulNdOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNMatmulNdOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNMatmulNdOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    CUDNNMatmulNdOp matmulndOp = dyn_cast<CUDNNMatmulNdOp>(op);

    auto *context = op->getContext();
    auto loc = op->getLoc();
    CUDNNMatmulNdOpAdaptor operandAdaptor(operands);
    ...
    auto callMatmulNd = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
        apiRegistry, CUDNNRuntimeAPI::API::CUDA_MATMUL_ND,
...
void mlir::populateCUDNNMatmulNdToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter) {
  patterns.insert<CUDNNMatmulNdOpLowering>(ctx, typeConverter);
}
