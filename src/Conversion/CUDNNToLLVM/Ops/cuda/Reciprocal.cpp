//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNReciprocalOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNReciprocalOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNReciprocalOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNReciprocalOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    CUDNNReciprocalOp reciprocalOp = dyn_cast<CUDNNReciprocalOp>(op);
    ...
    auto input = reciprocalOp.getX();
    auto output = reciprocalOp.getResult();
    auto inputDimAttr = reciprocalOp.getDimResult();
    ...
    auto callReciprocal = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
        apiRegistry, CUDNNRuntimeAPI::API::CUDA_RECIPROCAL,
        {extractInput, extractOutput, inputDim, rankConst});

    Value callReciprocalOutput = insertAndReturnOutputShapeInfo(
        context, loc, typeConverter, rewriter, op->getResult(0), callReciprocal);

    rewriter.replaceOp(op, callReciprocalOutput);

    return success();
  }
};

void mlir::populateCUDNNReciprocalToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter) {
  patterns.insert<CUDNNReciprocalOpLowering>(ctx, typeConverter);
}
