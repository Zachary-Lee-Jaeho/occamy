//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNGatherOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNGatherOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNGatherOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNGatherOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    CUDNNGatherOp gatherOp = dyn_cast<CUDNNGatherOp>(op);
    ...
    if(llvmElemType.dyn_cast_or_null<IntegerType>()) {
      callGather = CUDNNRuntimeAPI::callApi(rewriter, loc,
          apiRegistry, CUDNNRuntimeAPI::API::CUDA_GATHER_I64,
      ...
      callGather = CUDNNRuntimeAPI::callApi(rewriter, loc,
          apiRegistry, CUDNNRuntimeAPI::API::CUDA_GATHER_F32,
          {extractInput, inputDim, inputRankConst,
          extractOutput, outputDim, outputRankConst,
          extractIndices, indicesDim, indicesRankConst,
          axisConst});
...
void mlir::populateCUDNNGatherToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter) {
  patterns.insert<CUDNNGatherOpLowering>(ctx, typeConverter);
}
