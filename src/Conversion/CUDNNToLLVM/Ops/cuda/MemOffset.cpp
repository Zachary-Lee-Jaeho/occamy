//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNMemOffsetOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNMemOffsetOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNMemOffsetOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNMemOffsetOp::getOperationName(), ctx, typeConverter) {}
  ...
  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    auto callMemOffset = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
        apiRegistry, CUDNNRuntimeAPI::API::CUDA_MEMOFFSET,
...
void mlir::populateCUDNNMemOffsetToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter) {
  patterns.insert<CUDNNMemOffsetOpLowering>(ctx, typeConverter);
}
