//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNUnsqueezeOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNUnsqueezeOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNUnsqueezeOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNUnsqueezeOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    CUDNNUnsqueezeOp unsqueezeOp = dyn_cast<CUDNNUnsqueezeOp>(op);
    ...
    if(llvmElemType.dyn_cast_or_null<IntegerType>()) {
      callUnsqueeze = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
          apiRegistry, CUDNNRuntimeAPI::API::CUDA_UNSQUEEZE_I64,
          {extractInput, inputDim, extractOutput, outputDim, axes, rankConst});
    } else if(llvmElemType.dyn_cast_or_null<FloatType>()) {
      callUnsqueeze = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
          apiRegistry, CUDNNRuntimeAPI::API::CUDA_UNSQUEEZE_F32,
          {extractInput, inputDim, extractOutput, outputDim, axes, rankConst});
    }
...
void mlir::populateCUDNNUnsqueezeToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter) {
  patterns.insert<CUDNNUnsqueezeOpLowering>(ctx, typeConverter);
}
