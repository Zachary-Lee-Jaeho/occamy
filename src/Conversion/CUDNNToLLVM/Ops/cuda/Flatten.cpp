//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNFlattenOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNFlattenOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNFlattenOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNFlattenOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    CUDNNFlattenOp flattenOp = dyn_cast<CUDNNFlattenOp>(op);
    ...
    if(outputDimAttr.size() != 2)
      return emitError(loc, "CUDNNToLLVM: Flatten: only support 2D output");
    ...
    auto callFlatten = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
        apiRegistry, CUDNNRuntimeAPI::API::CUDA_FLATTEN,
...
void mlir::populateCUDNNFlattenToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter) {
  patterns.insert<CUDNNFlattenOpLowering>(ctx, typeConverter);
}
