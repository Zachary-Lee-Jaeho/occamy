//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNConcatOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNConcatOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNConcatOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNConcatOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    CUDNNConcatOp concatOp = dyn_cast<CUDNNConcatOp>(op);
    ...
    if(llvmElemType.dyn_cast_or_null<IntegerType>()) {
      ...
      callConcat = CUDNNRuntimeAPI::callApi(rewriter, loc,
          apiRegistry, CUDNNRuntimeAPI:: API::CUDA_CONCAT_I64,
      ...
      callConcat = CUDNNRuntimeAPI::callApi(rewriter, loc,
          apiRegistry, CUDNNRuntimeAPI:: API::CUDA_CONCAT_F32,
          {inputsArgs, inputDimsArgs, extractOutput, outputDim,
          axisConst, inNumConst, rankConst});
    }
...
void mlir::populateCUDNNConcatToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter) {
  patterns.insert<CUDNNConcatOpLowering>(ctx, typeConverter);
}
