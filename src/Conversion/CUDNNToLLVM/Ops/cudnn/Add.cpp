//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNAddOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNAddOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNAddOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNAddOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    CUDNNAddOp addOp = dyn_cast<CUDNNAddOp>(op);
    ...
    auto opModeConst = rewriter.create<LLVM::ConstantOp>(loc,
        int64Ty, rewriter.getI64IntegerAttr(0)/* CUDNN_OP_TENSOR_ADD*/);

    // TODO: Complete lowering without the library
    auto callAdd = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
...
void mlir::populateCUDNNAddToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter, Value handle) {
  patterns.insert<CUDNNAddOpLowering>(ctx, typeConverter);
