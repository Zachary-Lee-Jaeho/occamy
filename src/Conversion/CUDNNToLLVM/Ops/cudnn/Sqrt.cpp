//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNSqrtOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNSqrtOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNSqrtOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNSqrtOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    CUDNNSqrtOp sqrtOp = dyn_cast<CUDNNSqrtOp>(op);
    ...
    auto opModeConst = rewriter.create<LLVM::ConstantOp>(loc,
        int64Ty, rewriter.getI64IntegerAttr(4)/* CUDNN_OP_TENSOR_SQRT*/);

    // TODO: Complete lowering without the library
    auto callSqrt = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
        apiRegistry, CUDNNRuntimeAPI::API::CSRC_TENSOR_OP,
...
void mlir::populateCUDNNSqrtToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter, Value handle) {
  patterns.insert<CUDNNSqrtOpLowering>(ctx, typeConverter);
