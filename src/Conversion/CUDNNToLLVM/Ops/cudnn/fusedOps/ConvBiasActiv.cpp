//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNConvBiasActivOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNConvBiasActivOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNConvBiasActivOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNConvBiasActivForwardOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    CUDNNConvBiasActivForwardOp convbiasactivOp = dyn_cast<CUDNNConvBiasActivForwardOp>(op);
    ...
    // CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_PRECOMP_GEMM TODO: find algorithm in static time! not just 1
    auto algoConst = rewriter.create<LLVM::ConstantOp>(loc,
        int64Ty, rewriter.getI64IntegerAttr(1));

    // CUDNN Workspace Size
    ...
    auto callConv = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
        apiRegistry, CUDNNRuntimeAPI::API::CSRC_CONV_BIAS_RELUFWD,
...
void mlir::populateCUDNNConvBiasActivForwardToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter, Value handle) {
  patterns.insert<CUDNNConvBiasActivOpLowering>(ctx, typeConverter);
