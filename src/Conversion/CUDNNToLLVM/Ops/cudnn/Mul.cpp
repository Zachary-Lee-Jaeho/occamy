//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNMulOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNMulOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNMulOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNMulOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    CUDNNMulOp mulOp = dyn_cast<CUDNNMulOp>(op);
    ...
    auto opModeConst = rewriter.create<LLVM::ConstantOp>(loc,
        int64Ty, rewriter.getI64IntegerAttr(1)/* CUDNN_OP_TENSOR_MUL*/);

    // TODO: Complete lowering without the library
    auto callMul = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
...
void mlir::populateCUDNNMulToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter, Value handle) {
  patterns.insert<CUDNNMulOpLowering>(ctx, typeConverter);
