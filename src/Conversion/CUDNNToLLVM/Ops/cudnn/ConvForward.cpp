//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNConvForwardOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNConvForwardOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNConvForwardOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNConvForwardOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    CUDNNConvForwardOp convfwdOp = dyn_cast<CUDNNConvForwardOp>(op);
    ...
    // CUDNN Convolution group algorithm
    ...
    auto callConv = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
        apiRegistry, CUDNNRuntimeAPI::API::CSRC_CONVFWD,
...
void mlir::populateCUDNNConvForwardToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter, Value handle) {
  patterns.insert<CUDNNConvForwardOpLowering>(ctx, typeConverter);
