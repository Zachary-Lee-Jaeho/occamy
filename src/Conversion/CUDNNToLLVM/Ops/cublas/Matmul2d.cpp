//===----------------------------------------------------------------------===//
// CUDNN to LLVM: CUDNNMatmul2dOpLowering
//===----------------------------------------------------------------------===//
...
class CUDNNMatmul2dOpLowering : public ConvertToLLVMPattern {
public:
  CUDNNMatmul2dOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::CUDNNMatmul2dOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    CUDNNMatmul2dOp matmul2dOp = dyn_cast<CUDNNMatmul2dOp>(op);
    ...
    auto callMatmul2d = core_dnn::CUDNNRuntimeAPI::callApi(rewriter, loc,
        apiRegistry, core_dnn::CUDNNRuntimeAPI::API::CUBLAS_MATMUL_2D,
        {rankConst,
...
void mlir::populateCUDNNMatmul2dToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter) {
  patterns.insert<CUDNNMatmul2dOpLowering>(ctx, typeConverter);
}
