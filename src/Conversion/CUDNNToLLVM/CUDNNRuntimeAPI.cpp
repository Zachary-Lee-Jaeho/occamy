...
//===------ CUDNNRuntimeAPI.cpp - Implementation of Runtime API -----------===//
//
...
void CUDNNRuntimeAPI::declareAPI(ModuleOp &module, OpBuilder &builder) {
...
Value CUDNNRuntimeAPI::callApi(OpBuilder &builder, Location loc,
    const CUDNNRuntimeAPIRegistry &registry, API apiId, ArrayRef<Value> params) {
  ...
  const CUDNNRuntimeAPI &runtimeAPI = registry.getAPI(apiId);
...
CUDNNRuntimeAPIRegistry::~CUDNNRuntimeAPIRegistry() {}

CUDNNRuntimeAPIRegistry::CUDNNRuntimeAPIRegistry(ModuleOp &module, OpBuilder &builder,
    mlir::Type inType)
    : registry() {
  ...
  // clang-format off
  using API = CUDNNRuntimeAPI::API;
  #include "src/Conversion/CUDNNToLLVM/CUDNNAPISpecs.inc"
  // clang-format on
  ...
  for (auto &apiSpec : CUDNNRuntimeAPISpecs) {
