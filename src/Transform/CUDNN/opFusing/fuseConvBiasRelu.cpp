...
public:
  void runOnOperation() override {
    ...
    funcOp.walk([&](CUDNNConvForwardOp op) {
      fuseType_t opFuseType = CONVBIAS;
      CUDNNAddOp addOp = NULL;
      CUDNNActivationForwardOp activOp = NULL;
      if (!op->use_empty()) {
        for (Operation* user: op->getUsers()) {
          if (!(addOp = dyn_cast_or_null<CUDNNAddOp>(user))) {
      ...
      if(addOp) {
        if (!addOp->use_empty()) {
          for (Operation* user: addOp->getUsers()) {
            activOp = dyn_cast_or_null<CUDNNActivationForwardOp>(user);
            if (!activOp) {
              opFuseType = CONVBIAS;
              break;
            } else {
      ...
      if (opFuseType == CONVBIAS || opFuseType == CONVBIASRELU) {
        ...
        CUDNNConvForwardOpAdaptor convOpOperandAdaptor(convOpOperands);
        ...
        CUDNNAddOpAdaptor addOpOperandAdaptor(addOpOperands);
        ...
        if(opFuseType == CONVBIASRELU) {
          ...
          CUDNNActivationForwardOpAdaptor activOpOperandAdaptor(activOpOperands);
          ...
          auto fusedOp = builder.create<CUDNNConvBiasActivForwardOp>(
              ...
              builder.getI64IntegerAttr(1), // CUDNN_ACTIVATION_RELU
          ...
          CUDNNDeallocOp dummyDealloc;
          for (Operation* user: dummyMalloc->getUsers()) {
            if (isa<CUDNNDeallocOp>(user)) {
              dummyDealloc = dyn_cast_or_null<CUDNNDeallocOp>(user);
            }
          }
          ...
          auto fusedOp = builder.create<CUDNNConvBiasActivForwardOp>(
              ...
              bias, builder.getI64ArrayAttr(broadcastedBiasDim),
              workspace, builder.getI64IntegerAttr(workspaceSize), pads, strides,
              builder.getI64IntegerAttr(5), // CUDNN_ACTIVATION_IDENTITY
