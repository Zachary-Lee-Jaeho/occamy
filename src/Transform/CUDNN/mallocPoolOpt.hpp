#ifndef CUDNN_MALLOC_POOL_H
#define CUDNN_MALLOC_POOL_H
...
int64_t getMemRefSize(Operation* op) {
  Value memrefVal;
  if (isa<mlir::CUDNNMallocOp>(op))
    memrefVal = op->getResult(0);
  else if (isa<mlir::CUDNNDeallocOp>(op))
...
Operation* backupInterTensor (Operation* divOp, Operation* mallocOp) {
  ...
  builder.create<CUDNNMemcpyOp>(loc, int32Ty, alloc, mallocOp->getResult(0),
  ...
  builder.create<CUDNNDeallocOp>(loc, int32Ty, mallocOp->getResult(0));

  // copy back to device
  builder.setInsertionPointAfter(divOp);
  auto mallocBack = builder.create<CUDNNMallocOp>(loc, memrefType, sizeConst);
  builder.create<CUDNNMemcpyOp>(loc, int32Ty, mallocBack, alloc,
...
Operation* generateMemPoolInit (OpBuilder builder, int64_t& maxAllocByte, Operation* anchorOp) {
  ...
  auto memPoolInitOp = builder.create<mlir::CUDNNMemPoolInitOp>(loc,
...
void insertPoolDealloc (OpBuilder builder, Operation* poolInitOp) {
  ...
  auto int32Ty = builder.getIntegerType(32);
  CUDNNDeallocOp dealloc = builder.create<CUDNNDeallocOp>(loc, int32Ty, memPoolVal);
  dealloc.getOperation()->moveBefore(&funcOp.back().back());
  return;
}

#endif // CUDNN_MALLOC_POOL_H
