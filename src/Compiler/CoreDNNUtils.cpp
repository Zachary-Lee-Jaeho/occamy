...
void registerDialects(mlir::MLIRContext &context) {
  ...
  context.getOrLoadDialect<mlir::CUDNNDialect>();
}
...
static int emitOutputFiles(std::string outputNameNoExt,
  ...
  case EmitLib: {
    if (hardwareTarget == nvptx_cudnn) {
      addCompilerConfig(CCM_SHARED_LIB_DEPS, {"cd_cudnn_wrapper", "cudart", "cudnn", "cublas",  "cublasLt"});
    }
...
int compileModule(mlir::OwningOpRef<ModuleOp> &module,
  ...
  if (emissionTarget >= EmitMLIR) {
    if (hardwareTarget == nvptx_cudnn) {
      std::cout << "ONNX dialect --> CUDNN dialect" << std::endl;
      addONNXToCUDNNPasses(pm);

      if (cudnnKernelFusion)
        pm.addPass(createfuseConvBiasActivPass());
      if (cudnnDeallocOpt)
        pm.addPass(createCUDNNDeallocOptPass());
      if (mallocPoolOpt)
  ...
  if (inputIRLevel <= LLVMLevel && emissionTarget >= EmitLLVMIR) {
    if (hardwareTarget == nvptx_cudnn) {
      std::cout << "CUDNN dialect --> LLVM dialect" << std::endl;
      addCUDNNToLLVMPasses(pm);
